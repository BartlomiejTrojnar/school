...
class ExamDescriptionController extends Controller 
{
    public function index(ExamDescriptionRepository $examDescriptionRepo, SubjectRepository $subjectRepo, SessionRepository $sessionRepo) {
        ...
        return view('examDescription.index', ["examDescriptions"=>$examDescriptions, "sessionSF"=>$sessionSF, "subjectSF"=>$subjectSF, "examTypeSF"=>$examTypeSF, "levelSF"=>$levelSF]);
    }
    ...
    public function create(Request $request, SubjectRepository $subjectRepo, SessionRepository $sessionRepo) {
        ...
        if($request->version=="forSession")
            return view('examDescription.createForSession', ["session_id"=>$request->session_id, "subjectSF"=>$subjectSF, "examTypeSF"=>$examTypeSF, "levelSF"=>$levelSF]);

        $sessions = $sessionRepo -> getAllSorted();
        $sessionSelected = session()->get('sessionSelected');
        $sessionSF = view('session.selectField', ["sessions"=>$sessions, "sessionSelected"=>$sessionSelected]);
        return view('examDescription.create', ["sessionSF"=>$sessionSF, "subjectSF"=>$subjectSF, "examTypeSF"=>$examTypeSF, "levelSF"=>$levelSF]);
    }
    ...
    public function show($id, ExamDescriptionRepository $examDescriptionRepo, ClassroomRepository $classroomRepo, DeclarationRepository $declarationRepo, ExamRepository $examRepo, TermRepository $termRepo, SessionRepository $sessionRepo, $view='') {
        if(empty(session()->get('examDescriptionView')))  session()->put('examDescriptionView', 'info');
        ...
        switch(session()->get('examDescriptionView')) {
            case 'info':        return $this -> showInfo($examDescription);
            case 'terminy':     return $this -> showTerms($examDescription, $classroomRepo, $termRepo, $sessionRepo);
            case 'egzaminy':    return $this -> showExams($examDescription, $declarationRepo, $termRepo, $examRepo);
            default:
    ...
    private function showTerms($examDescription, $classroomRepo, $termRepo, $sessionRepo) {
        ...
        $sessionSelected = session()->get('sessionSelected');
        $sessionSF = view('session.selectField', ["sessions"=>$sessions, "sessionSelected"=>$sessionSelected]);
        $termTable = view('term.table', ["terms"=>$terms, "sessionSF"=>$sessionSF, "examDescriptionSF"=>"", "classroomSF"=>$classroomSF]);
    ...
    private function showExams($examDescription, $declarationRepo, $termRepo, $examRepo) {
        ...
        $declarationSF = view('declaration.selectField', ["declarations"=>$declarations, "declarationSelected"=>$declaration_id]);

        $terms = $termRepo -> getFilteredAndSorted($exam_description_id, $classroom_id, $session_id);
        $termSF = view('term.selectField', ["terms"=>$terms, "termSelected"=>$term_id]);
        ...
        $examTable = view('exam.table', ["exams"=>$exams, "countExams"=>$countExams, "declarationSF"=>$declarationSF, "termSF"=>$termSF, "examTypeSF"=>$examTypeSF, "version"=>"forExamDescription"]);
    ...
    public function edit(Request $request, ExamDescription $examDescription, SessionRepository $sessionRepo, SubjectRepository $subjectRepo) {
        ...
        $subjects = $subjectRepo -> getActualAndSorted();
        $subjectSF = view('subject.selectField', ["subjects"=>$subjects, "subjectSelected"=>$examDescription->subject_id]);
        $examTypes = array('pisemny', 'ustny');
        $examTypeSF = view('examDescription.examTypeSelectField', ["examTypes"=>$examTypes, "examTypeSelected"=>$examDescription->type]);
        $levels = array('podstawowy', 'rozszerzony', 'nieokreślony', 'dwujęzyczny');
        $levelSF = view('layouts.levelSelectField', ["levels"=>$levels, "levelSelected"=>$examDescription->level]);
        if($request->version=="forSession")
            return view('examDescription.editForSession', ["examDescription"=>$examDescription, "subjectSF"=>$subjectSF, "examTypeSF"=>$examTypeSF, "levelSF"=>$levelSF]);

        $sessions = $sessionRepo -> getAllSorted();
        $sessionSF = view('session.selectField', ["sessions"=>$sessions, "sessionSelected"=>$examDescription->session_id]);
        return view('examDescription.edit', ["examDescription"=>$examDescription, "sessionSF"=>$sessionSF, "subjectSF"=>$subjectSF, "examTypeSF"=>$examTypeSF, "levelSF"=>$levelSF]);
    }
