...
function showCreateTable() {
    $.ajax({
        ...
        success: function(result) { $('#declarations footer').append(result).hide().show(500);  },
        error: function() {
            var error = '<p class="error">Błąd tworzenia wiersza z formularzem dodawania deklaracji.</p>';
            $('#declarations footer').append(error).hide().show(1000);
        },
    });
}
...
function addClick() {     // ustawienie instrukcji po kliknięciu anulowania lub potwierdzenia dodawania deklaracji
    $('#declarations').delegate('#cancelAdd', 'click', function() {
        $.when( $('#createTable').hide(500) ).then(  function() {
            $('#showCreateTable').show(500);
            $('#createTable').remove();
         });
    });
...
function add() {   // zapisanie deklaracji w bazie danych
    var student_id          = $('#createTable input[name="student_id"]').val();
    var session_id          = $('#createTable select[name="session_id"]').val();
    var application_number  = $('#createTable input[name="application_number"]').val();
    var student_code        = $('#createTable input[name="student_code"]').val();
    ...
    $.ajax({
        ...
        success: function(id) {  refreshDeclaration(id, 1);  },
        error: function() {
            var error = '<p class="error">Błąd tworzenia wiersza z formularzem dodawania deklaracji.</p>';
            $('#declarations footer').append(error).hide().show(1000);
        },
