...
function refreshRow(id, version, lp=0) {  // odświeżenie wiersza z opisem egzaminem o podanym identyfikatorze
    $.ajax({
        ...
        data: { id: id, version: "forSession", lp: lp },
        ...
        error: function() {
            var error = '<tr><td class="error" colspan="8">Błąd odświeżania wiersza z opisem egzaminu!</td></tr>';
...
function showCreateRowClick() {
    $('#showCreateRow').click(function(){
        $(this).hide();
        $('table#examDescriptions').animate({width: '1500px'}, 500);
        var session_id = $('input#session_id').val();
        showCreateRow(session_id);
        return false;
    });
}

function showCreateRow(session_id) {
    $.ajax({
        method: "GET",
        headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
        url: "http://localhost/school/opis_egzaminu/create",
        data: { session_id: session_id, version: "forSession" },
        success: function(result) { $('#examDescriptions').append(result); },
        error: function() {
            var error = '<tr><td colspan="8" class="error">Błąd tworzenia wiersza z formularzem dodawania opisu egzaminu.</td></tr>';
...
function add() {   // zapisanie opisu egzaminu w bazie danych
    var session_id  = $('#createRow input[name="session_id"]').val();
    ...
    $.ajax({
        ...
        error: function() {
            var error = '<tr><td colspan="8" class="error">Błąd dodawania opisu egzaminu.</td></tr>';
...
function editClick() {     // kliknięcie przycisku modyfikowania opisu egzaminu
    $('#examDescriptions').delegate('button.edit', 'click', function() {
        ...
        $.ajax({
            ...
            data: { id: id, version: "forSession" },
            success: function(result) {
                $('tr[data-exam_description_id='+id+']').before(result).hide();
                updateClick(lp);
            },
            error: function() {
                var error = '<tr><td colspan="8" class="error">Błąd tworzenia wiersza z formularzem modyfikowania opisu egzaminu.</td></tr>';
...
function update(id, lp) {   // zapisanie opisu egzaminu w bazie danych
    var session_id  = $('tr[data-exam_description_id='+id+'] input[name="session_id"]').val();
    ...
    $.ajax({
        ...
        error: function() {
            var error = '<tr><td colspan="8" class="error">Błąd modyfikowania opisu egzaminu.</td></tr>';
...
function destroyClick() {  // usunięcie opisu egzaminu (z bazy danych)
    $('#examDescriptions').delegate('.destroy', 'click', function() {
        ...
        $.ajax({
            ...
            error: function() {
                var error = '<tr><td colspan="8" class="error">Błąd usuwania opisu egzaminu.</td></tr>';
                $('tr[data-exam_description_id='+id+']').after(error).hide();
            }
        });
        return false;
