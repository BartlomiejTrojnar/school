...
function refreshRow(id, lp, operation="add", success="true") {   // odświeżenie wiersza z informajami o rekordzie o podanym identyfikatorze
    ...
    if(!success)
        switch(operation) {
            case "update": RefreshRow.updateError(id, "Błąd w trakcie modyfikowania szkoły."); break;
            case "add": RefreshRow.addError("Błąd w trakcie dodawania szkoły."); return;
            case "destroy": RefreshRow.destroyError(id, "Nie można usunąć szkoły. Prawdopodobnie istnieją powiązane rekordy."); return;
        }
    ...
    $.ajax({
        ...
        error: function() { RefreshRow.error(id, operation, "Błąd odświeżania wiersza z szkoły."); },
    });
}
...
function clickCreateRowButtons() {   // kliknięcie przycisku dodawania
    $('#showCreateRow').click(function(){       // tworzenie formularza dodawania
        $('tr#createRow').remove();
        var communique = "Błąd tworzenia wiersza z formularzem dodawania szkoły.";
...
function clicEditRowButtons() {
    $(TABLE_NAME).delegate('button.edit', 'click', function() {     // tworzenie formularza modyfikowania
        var communique = "Nie mogę utworzyć formularza do zmiany danych szkoły.";
