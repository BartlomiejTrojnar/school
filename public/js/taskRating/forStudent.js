...
*/
function refreshRow(id, lp) {  // odświeżenie wiersza z oceną zadania
    $.ajax({
        method: "POST",
        headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
        ...
        data: { id: id, lp: lp },
        success: function(result) {
            $('tr[data-task_rating_id='+id+']').replaceWith(result);
        },
        error: function() {
            var error = '<tr><td class="error" colspan="13">Błąd odświeżania tabeli z ocenami zadań dla ucznia: Dokończyć funkcję odświeżającą tabelę z ocenami zadań dla ucznia.</td></tr>';
