...
<table>
   ...
   <tr>
      ...
      <td class="teachers" style="min-width: 400px;">
         <aside style="float: right; margin-left: 10px;"><a href="{{ url('grupa_nauczyciele/addTeacher/'. $group->id) }}">
  ...
  <tr>
      ...
      <td class="grades">
         ...
         @foreach($grades as $groupGrade)
            <div data-groupGrade_id="{{ $groupGrade->id }}">
               <a href="{{ route('klasa.show', $groupGrade->grade_id) }}" class="no-students" data-grade_id="{{ $groupGrade->grade_id }}">
                  @if( $year )
   ...
   <tr>
      ...
      <td class="c">{{ $group->students->count() }}
         <ol id="groupStudents" hidden>@foreach($group->students as $gs)
