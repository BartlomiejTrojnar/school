...
<table id="students">
    ...
    <tr>
      ...
      <td>
        @foreach($student->grades as $studentGrade)
          {{ $studentGrade->grade->year_of_beginning }}-{{ $studentGrade->grade->year_of_graduation }} {{ $studentGrade->grade->symbol }}<br />
        @endforeach
      </td>
  ...
  @endforeach
    ...
    <tr class="create">
      <td colspan="13"><a href="{{ route('uczen.create') }}"><img class="create" src="{{ asset('css/plus.png') }}" /></a></td>
    </tr>
  </tbody>
</table>
