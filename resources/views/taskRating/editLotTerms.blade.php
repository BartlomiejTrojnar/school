...
@section('main-content')
  ...
  <form action="{{ route('ocena_zadania.updateLotTerms') }}" method="post" role="form">
  {{ csrf_field() }}
    <table>
      <tr>
        <th>lp</th>
      ...
      @foreach($taskRatings as $taskRating)
        <tr class="unlocked" data-taskRating="{{$taskRating->id}}">
          ...
          <td><input type="date" class="valueInput" name="deadline{{$taskRating->id}}" value="{{ substr($taskRating->deadline, 0, 10) }}" /></td>
